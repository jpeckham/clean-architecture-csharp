...
public sealed class ViewUserPresenter : IViewUserOutputBoundary
{
    ...
    private static ViewUserPostSummaryViewModel ToViewModel(ViewUserPostSummaryResponse post) =>
        new(
            post.Id,
            post.AuthorHandle,
            SegmentContent(post.Content, post.Mentions, post.Hashtags),
            post.ParentPostId,
            post.OriginalPostId,
            post.CreatedAt,
            post.LikeCount,
            ...
            post.QuotedPost is null
                ? null
                : new ViewUserQuotedPostSummaryViewModel(post.QuotedPost.Id, post.QuotedPost.AuthorHandle, post.QuotedPost.Content, post.QuotedPost.CreatedAt, post.QuotedPost.Media?.Select(m => new ViewUserPostMediaSummaryViewModel(m.AssetId, m.Kind, m.ContentType, m.ByteLength, m.Width, m.Height, m.DurationMs, m.SortOrder, m.ThumbnailKey, m.AltText)).ToArray()),
    ...
    private static IReadOnlyList<UserPostContentSegment> SegmentContent(string content, IReadOnlyList<string>? mentions, IReadOnlyList<string>? hashtags)
    {
